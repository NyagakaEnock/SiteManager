...
Private Sub Command1_Click()
    Dim rscr As New ADODB.Recordset
     strSQL = "ALTER TABLE ODASMLeaseAgreement ADD Reason text"
     rscr.Open strSQL, cnCOMMON, adOpenKeyset, adLockOptimistic
End Sub

Private Sub Form_Load()
      getContractToTerminated
      strSQL = "SELECT LA.COntractNo,LA.PlotNo,A.COmpanyName,COmmencementdate,ExpiryDate,LeaseDuration,Terminated,TerminationDate,LA.Reason FROM ODASMLeaseAgreement LA INNER JOIN ODASPAccount A ON A.AccountNo=LA.AccountNo "
            ...
            Case "S"
                Select Case Button.Caption
                    Case "&Search/Find "
                        ...
                        strSQL = "SELECT LA.COntractNo,LA.PlotNo,A.COmpanyName,COmmencementdate,ExpiryDate,LeaseDuration,Terminated,TerminationDate,LA.Reason FROM ODASMLeaseAgreement LA INNER JOIN ODASPAccount A ON A.AccountNo=LA.AccountNo WHERE PlotNo LIKE '" & CurrentRecord & "'"
                        FillList strSQL, Me.ListView1
...
strSQL = "UPDATE ODASMLeaseAgreement SET Terminated='Y',TerminationDate='" & Format(Date, "yyyy/MM/dd") & "',TerminatedBy='" & CurrentUserName & "',Reason='" & txtreason & "' WHERE ContractNo LIKE '" & Me.txtContractNo & "'"
