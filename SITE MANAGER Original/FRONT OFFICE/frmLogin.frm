            ...
            If ActiveUserProfile Then
                If HasModuleRights Then
                    ...
                    If ValidPassword Then
                        ...
                        ALISFOManager.StatusBar1.Panels(2).Text = "CURRENT USER: " & " " & UCase(frmLogin.txtUserName.Text): ALISFOManager.StatusBar1.Panels(3).Text = "LOG ON TIME: " & " " & LTime: ALISFOManager.StatusBar1.Panels(4).Text = "Computer Name: " & " " & GetCompName
                        ALISFOManager.mnuFileEndSession.Caption = "Log Off " & CurrentUserName & " ..."
                        Load ALISFOManager: Unload Me
                        ALISFOManager.Show vbModal
                    Else
...
Private Function HasModuleRights() As Boolean
On Error GoTo err
With Me
    If UserIsAdministrator Then
        HasModuleRights = True
    Else
        Set rsFindRecord = cnSECURE.Execute("SELECT UserModules.allow FROM UserModules WHERE UserName='" & Trim(.txtUserName.Text) & "' AND ModuleID='" & App.EXEName & "';")
        If rsFindRecord.EOF And rsFindRecord.BOF Then
...
With Me
 ...
 expirydate = "10/05/2006"
 expirydate = Format(expirydate, "dd/mm/yyyy")
  'If Format(Date, "dd/mm/yyyy") >= expirydate Then Exit Sub
