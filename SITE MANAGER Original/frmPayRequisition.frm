...
Begin VB.Form frmPayRequisition 
   ...
   End
End
Attribute VB_Name = "frmPayRequisition"
...
strSQL = "UPDATE ODASMVoucher SET Printed='Y', DatePrinted='" & Format(Date, "yyyy/MM/dd") & "', PrintedBy='" & CurrentUserName & "' WHERE VoucherNo = '" & CurrentRecord & "'"
...
Private Sub Form_Load()
    ...
    If rsFindRecord.EOF And rsFindRecord.BOF Then Exit Sub
        If rsFindRecord!Printed = "Y" Then
            'CRViewer1.EnablePrintButton = True
            Report.txtCopy.Suppress = False
        Else
            'CRViewer1.EnablePrintButton = False
            Report.txtCopy.Suppress = True
        End If
    Set rsFindRecord = Nothing
Screen.MousePointer = vbHourglass: Dim myCrystalLogon As New clsCrystallogon
Report.RecordSelectionFormula = "{ODASMVoucher.VoucherNo}= '" & CurrentRecord & " '" ' OR ({ODASMVoucher.VoucherDate}=date('" & Format(frmODASMVoucher.txtRequisitionDate.Text, "yyyy,MM,dd") & "'));"
