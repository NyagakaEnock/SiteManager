...
If frmODASSearchSitesNotPaid.txtStartDate.Visible = True Then
    ...
    a = "{ODASMInstallment.PaymentDueDate} >= DateTime('" & Format(CDate(frmODASSearchSitesNotPaid.txtStartDate.Value), "mm/dd/yyyy") & "') and {ODASMInstallment.PaymentDueDate} <= DateTime('" & Format(CDate(frmODASSearchSitesNotPaid.txtLastDate.Value), "mm/dd/yyyy") & "') And  (ISNULL({ODASMLeaseAgreement.Terminated})  OR {ODASMLeaseAgreement.Terminated}='N' )"
    'a = "SELECT * FROM ODASMInstallment,ODASPPlot,ODASPAccount Where (ODASMInstallment.PaymentDueDate >= '" & Format(frmODASSearchSitesNotPaid.txtStartDate.Value, "yyyy/mm/dd") & "' AND ODASMInstallment.PaymentDueDate <= '" & Format(frmODASSearchSitesNotPaid.txtLastDate.Value, "yyyy/mm/dd") & "') and ODASMInstallment.PaymentFlag = 'N' and ODASMInstallment.Balance > 0 and ODASPPlot.PlotNo=ODASMInstallment.PlotNo and ODASMInstallment.AccountNo=ODASPAccount.AccountNo AND ODASMInstallment.ContractNo IN (Select ContractNo From ODASMLeaseAgreement where Terminated='N' OR Terminated IS NULL);"
...
With frmODASSearchSitesNotPaid
    Select Case .strReport
        ...
        Case "PendingPaymentAsAtASingleDate"
            a = "{ODASMInstallment.PaymentDueDate} <= DateTime('" & Format(CDate(frmODASSearchSitesNotPaid.txtLastDate.Value), "mm/dd/yyyy") & "') and (ISNULL({ODASMLeaseAgreement.Terminated})  OR {ODASMLeaseAgreement.Terminated}='N' )"
            Set Report = New ODASRRentDue
...
Report.RecordSelectionFormula = a
